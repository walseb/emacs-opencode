...
- Session UI
  - Session buffers are backed by model structs and maintain message parts,
    a stable input boundary marker, and header rendering state.  The editable
    input region runs from that boundary to the end of the buffer, so streaming
    log updates can insert before it without consuming typed input.
  - Incremental updates from streaming events mutate messages and re-render
    affected regions in place while preserving window point and scroll state for
    users reading in-progress output.
- Models and caches
